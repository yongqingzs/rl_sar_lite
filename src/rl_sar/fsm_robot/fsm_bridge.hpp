...
#ifndef BRIDGE_FSM_HPP
#define BRIDGE_FSM_HPP
...
public:
    ...
    std::string CheckChange() override
    {
        ...
        if (percent_getup >= 1.0f)
        {
            ...
            {
                return "RLFSMStateRLLocomotion1";
            }
...
class RLFSMStateRLLocomotion1 : public RLFSMState
{
public:
    RLFSMStateRLLocomotion1(RL *rl) : RLFSMState(*rl, "RLFSMStateRLLocomotion1") {}
    ...
    void Enter() override
    {
        ...
        // read params from yaml
        rl.config_name = "legged_gym";
        std::string model_folder = rl.params.Get<std::string>("model_folder1");
        std::cout << LOGGER::INFO << "model_folder1: " << model_folder << std::endl;
        if (!model_folder.empty()) {
            rl.config_name = model_folder;
        }
        std::cout << LOGGER::NOTE << "true config_name: " << rl.config_name << std::endl;
...
public:
    ...
    void Enter() override
    {
        ...
        // read params from yaml
        rl.config_name = "legged_gym";
        std::string model_folder = rl.params.Get<std::string>("model_folder2");
        std::cout << LOGGER::INFO << "model_folder2: " << model_folder << std::endl;
        if (!model_folder.empty()) {
            rl.config_name = model_folder;
        }
        std::cout << LOGGER::NOTE << "true config_name: " << rl.config_name << std::endl;
...
class BridgeFSMFactory : public FSMFactory
{
public:
    BridgeFSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<bridge_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<bridge_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<bridge_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLLocomotion")
            return std::make_shared<bridge_fsm::RLFSMStateRLLocomotion>(rl);
        else if (state_name == "RLFSMStateRLLocomotion1")
            return std::make_shared<bridge_fsm::RLFSMStateRLLocomotion1>(rl);
        else if (state_name == "RLFSMStateRLLocomotion2")
            return std::make_shared<bridge_fsm::RLFSMStateRLLocomotion2>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "bridge"; }
...
REGISTER_FSM_FACTORY(BridgeFSMFactory, "RLFSMStatePassive")

#endif // BRIDGE_FSM_HPP
